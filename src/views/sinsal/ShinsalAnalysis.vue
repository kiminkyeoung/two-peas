...
import { checkSinsal } from '../../utils/sinsalLogic.js'
...
const handleSubmit = () => {
  ...
  const hour = isTimeUnknown.value ? 12 : (birthHour.value ? parseInt(birthHour.value) : 12);
  const minute = isTimeUnknown.value ? 0 : (birthMinute.value ? parseInt(birthMinute.value) : 0);

  try {
    const foundSals = checkSinsal(dateStr, hour, minute);
    
    const result = {
      birthDate: dateStr,
      birthTime: isTimeUnknown.value ? '모름' : `${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}`,
      sals: foundSals
    };
    ...
    console.error("신살 분석 중 오류:", error);
    alert("분석 중 오류가 발생했습니다. 입력값을 다시 확인해주세요.");
  }
};
</script>
    ...
    <section class="hero">
      <div class="hero__intro center">
        ...
        <p class="hero__subtitle">
          타고난 사주에 어떤 기운들이 숨어있는지<br/>내 사주 속 신살(神煞)을 분석해드립니다.
        </p>
      </div>
